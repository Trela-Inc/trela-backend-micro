...
export class EventBus {
  ...
  async connect(): Promise<void> {
    try {
      ...
      await this.channel.assertQueue('notification.bulk.send', { durable: true });
      await this.channel.assertQueue('notification.template.create', { durable: true });
      await this.channel.assertQueue('notification.preferences.update', { durable: true });
      
      // Bind queues to exchanges
      await this.channel.bindQueue('notification.send', 'notification.events', 'send');
      await this.channel.bindQueue('notification.bulk.send', 'notification.events', 'bulk.send');
      await this.channel.bindQueue('notification.template.create', 'notification.events', 'template.create');
      await this.channel.bindQueue('notification.preferences.update', 'notification.events', 'preferences.update');
